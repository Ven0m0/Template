...
1. **Integ**: Real deps (`services`: db/redis); run after unit.
1. **E2E**: Cypress/Playwright vs staging; mitigate flake (retries/waits).
1. **Perf**: Load test (k6/JMeter) crit paths; check baselines.
1. **Reports**: Upload JUnit/HTML as artifacts; use PR annotations.
